...
# from alignn.models.alignn_without_pgraph import ALIGNNConfig
...
class TrainingConfig(BaseSettings):
    ...
    # model configuration
    # NOTE
    # FIX: changed the default from ALIGNNAtomWiseConfig(name="alignn_atomwise")
    # to ALIGNNConfig(name="alignn").
    #
    # The original default was ALIGNNAtomWise, which caused two problems:
    #   1. config.model resolved to ALIGNNAtomWiseConfig even when training
    #      the plain ALIGNN, so setting config.model.pore_features = 5 raised
    #      a pydantic ValueError ("object has no field pore_features") even
    #      though pore_features is declared in ALIGNNConfig.
    #   2. _model.get(config.model.name) in train.py would return None for
    #      "alignn_atomwise" since we removed that key from the dict, causing
    #      a silent failure.
    #
    # Changing the default to ALIGNNConfig means pydantic instantiates the
    # right class from the start and all ALIGNNConfig fields (including
    # pore_features) are immediately available without any extra steps.
    #
