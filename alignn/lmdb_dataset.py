...
class TorchLMDBDataset(Dataset):
    ...
    def collate_line_graph(
        ...
        PORE: handles both the original 4-element tuple:
            (graph, line_graph, lattice, label)
        and the pore-augmented 5-element tuple from PoreAugmentedDataset:
            (graph, line_graph, lattice, pore_feats, label)

        When pore_feats are present they are stacked into a single tensor
        and included in the return tuple just before the labels, matching
        the layout that _unpack_batch() in train.py expects:
            (batched_graph, batched_line_graph, lattices, pore_feats, labels)
        """
        # Unpack dynamically so this works for both 4- and 5-element tuples.
        # OLD code (kept for reference, crashes when pore tensor is present):
        # graphs, line_graphs, lattices, labels = map(list, zip(*samples))
        unpacked = list(map(list, zip(*samples)))

        if len(unpacked) == 5:
            # PORE: 5-tuple, (graph, line_graph, lattice, pore_feats, label)
            graphs, line_graphs, lattices, pore_feats, labels = unpacked
        else:
            # Original 4-tuple, (graph, line_graph, lattice, label)
            graphs, line_graphs, lattices, labels = unpacked
            pore_feats = None
        ...
        if pore_feats is not None:
            # PORE: include the stacked pore tensor between lattices and labels
            return (
                batched_graph,
                batched_line_graph,
                torch.stack(lattices),
                torch.stack(pore_feats),   # (batch_size, n_pore)
                batched_labels,
            )
        else:
            # Original return, identical to the old code
            return (
                batched_graph,
                batched_line_graph,
                torch.stack(lattices),
