...
# import numpy as np
import time
from alignn.train_with_fix import train_dgl
...
def train_prop_model(
    ...
    # NOTE:
    # train_dgl() does not return any value (returns None by default).
    # The original code attempted to access result["train"] and result["validation"],
    # which caused a crash (TypeError: 'NoneType' object is not subscriptable).
    # Therefore, we removed usage of 'result' and rely on internal logging/output instead.
    # t1 = time.time()
    ...
    # 
    # NOTE:
    # train_dgl() is a side-effect training loop (returns None).
    # Do not attempt to capture return values.
    # All outputs are saved via:
    # - history_train.json
    # - history_val.json
    # - model checkpoints (best_model.pt, last_model.pt)
    # Training progress is logged internally, not returned.



    #testing
    print("\n[DEBUG] CONFIG MODEL SETTINGS")
    print("ATOM FEATURES:", config["model"].get("atom_input_features"))
    print("HIDDEN FEATURES:", config["model"].get("hidden_features"))
    print("MODEL NAME:", config["model"].get("name"))
    print("FULL MODEL CONFIG:", config["model"])
