...
from alignn.models.alignn_without_pgraph import ALIGNN
...
def _unpack_batch(dats, compute_line_graph: bool, has_pore: bool, device):
    """Unpack a loader batch into (graphs, pore_feats, target).

    Parameters
    ----------
    dats               : raw tuple from the DataLoader
    compute_line_graph : True when config.compute_line_graph > 0
    has_pore           : True when config.model.pore_features > 0
    device             : torch device

    Returns
    -------
    graphs     : list [g, lg, lat] or [g, lg]  (already on device)
    pore_feats : Tensor (batch, n_pore) on device, or None
    ...
    if compute_line_graph:
        ...
        if has_pore:
            # (g, lg, lat, pore, target)
            pore = dats[3].to(device).float()
        else:
            pore = None
        graphs = [g, lg, lat]
    else:
...
def train_dgl(
    ...
    #This line was wrong.
    # FIX: the plain ALIGNN model returns a raw tensor, not a {"out": ...} dict
    # like ALIGNNAtomWise does. Normalise all forward calls through this helper
    # so the rest of the loop can always do result["out"] regardless of model.
    ...
    def forward(net, inputs, pore_feats=None):
        result = net(inputs, pore_feats=pore_feats)
    ...
    if "alignn" in config.model.name:
        ...
        for e in range(config.epochs):
            ...
            for dats, jid in zip(train_loader, train_loader.dataset.ids):
                ...
                graphs, pore, target_train = _unpack_batch(
                    dats,
                    compute_line_graph=config.compute_line_graph > 0,
                    has_pore=has_pore,
                    device=device,
                )
                result = forward(net, graphs, pore_feats=pore)
            ...
            for dats, jid in zip(val_loader, val_loader.dataset.ids):
                ...
                graphs, pore, target_val = _unpack_batch(
                    dats,
                    compute_line_graph=config.compute_line_graph > 0,
                    has_pore=has_pore,
                    device=device,
        ...
        if rank == 0 or world_size == 1:
            ...
            with torch.no_grad():
                for dats, jid in zip(test_loader, test_loader.dataset.ids):
                    ...
                    #     result = forward(net, [dats[0].to(device), dats[1].to(device)])
                    #     # result = net(dats[0].to(device))
                    graphs, pore, target_test = _unpack_batch(
                        dats,
                        compute_line_graph=config.compute_line_graph > 0,
                        has_pore=has_pore,
                        device=device,
        ...
        if config.write_predictions and classification:
            ...
            with torch.no_grad():
                ...
                for dat, id in zip(test_loader, ids):
                    ...
                    graphs, pore, target = _unpack_batch(
                        dat,
                        compute_line_graph=config.compute_line_graph > 0,
                        has_pore=has_pore,
                        device=device,
